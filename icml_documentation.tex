...
\large Comprehensive Technical Documentation (v4.0 --- TCN Primary Model)}
...
%=============================================================================
\section{TCN-Based I-V Reconstruction (Primary Model)}
\label{sec:tcn}
%=============================================================================

\textbf{Key Finding:} The Temporal Convolutional Network (TCN) approach \textbf{significantly outperforms} the Split-Spline architecture. Dilated causal convolutions \textbf{without} self-attention is the best-performing configuration.

\subsection{TCN Architecture (PhysicsIVSystem)}

The TCN model is implemented in PyTorch Lightning (\texttt{train\_attention\_tcn.py}). It operates directly on 8-point PCHIP voltage slices focused around the MPP region.

\begin{equation}
\mathbf{x} \xrightarrow{\text{ParamMLP}} \mathbf{h} \xrightarrow{\text{broadcast}} [\mathbf{h}; \text{RBF}(V)] \xrightarrow{\text{DilatedTCN}} \xrightarrow{\text{OutHead}} \hat{I}(V)
\end{equation}
...
\begin{table}[h]
\centering
\caption{TCN Parameter MLP Architecture}
\begin{tabular}{lccccc}
\toprule
\textbf{Layer} & \textbf{Input} & \textbf{Output} & \textbf{Activation} & \textbf{Norm} & \textbf{Dropout} \\
\midrule
...
\subsubsection{Temporal Convolution Blocks}

The sequence representation $[\mathbf{h}; \text{RBF}(V)] \in \mathbb{R}^{8 \times (128+128)}$ passes through dilated causal 1D convolution blocks:

\begin{table}[h]
\centering
\caption{TCN Block Architecture}
\begin{tabular}{lcccc}
\toprule
\textbf{Block} & \textbf{Filters} & \textbf{Kernel} & \textbf{Dilation} & \textbf{Padding} \\
\midrule
Block 1 & 128 & 5 & 1 & Causal (left-padded) \\
Block 2 & 64 & 5 & 2 & Causal (left-padded) \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Causal padding}: Left-only zero-padding of size $(k-1) \times d$ ensures each output position depends only on current and previous voltage positions.

\subsubsection{Architecture Variants}

Three variants were evaluated:
\begin{itemize}
    \item \textbf{TCN (causal dilated)}: Best performer. Dilation captures multi-scale voltage dependencies. Causal structure reflects the physical monotonic relationship.
    \item \textbf{Conv (non-causal 1D)}: Symmetric padding. Slightly worse than causal TCN.
...
An optional multi-head self-attention block (4 heads, $d_{model} = 64$) can be placed after the conv blocks. \textbf{Result: Attention consistently hurts performance.} Hypothesis: the 8-point PCHIP sequence is too short for attention to add value; dilation already provides sufficient receptive field.

\subsection{TCN Physics Loss}

\begin{equation}
\mathcal{L}_{TCN} = \mathcal{L}_{MSE} + \lambda_{mono} \mathcal{L}_{mono} + \lambda_{conv} \mathcal{L}_{conv} + \lambda_{curv} \mathcal{L}_{curv}
\end{equation}
...
\begin{itemize}
    ...
    \item Fine-grid (1000-point) ground truth stored as float16 memmaps for visualization
    \item Train/val/test: 70/15/15, stratified by $V_{oc}$ bins
\end{itemize}

\subsection{TCN Training Configuration}

\begin{table}[h]
\centering
\caption{TCN Training Configuration}
\begin{tabular}{lll}
\toprule
\textbf{Component} & \textbf{Parameter} & \textbf{Value} \\
\midrule
...
\subsection{TCN vs Split-Spline Comparison}

\begin{table}[h]
\centering
\caption{TCN vs Split-Spline Trade-offs}
\begin{tabular}{lll}
\toprule
\textbf{Aspect} & \textbf{TCN} & \textbf{Split-Spline} \\
\midrule
Performance & Significantly better & Baseline \\
...
\subsection{Tier 0: Must-Have Ablations (Main Paper --- TCN Pipeline)}

\begin{table}[h]
\centering
\caption{Tier 0 TCN Ablation Matrix (3 seeds each)}
\begin{tabular}{llp{6cm}}
\toprule
\textbf{ID} & \textbf{Configuration} & \textbf{Tests Claim} \\
\midrule
T0-1 & Dilated TCN, no attention & \textbf{Main model} (best configuration) \\
T0-2 & Standard Conv, no attention & Effect of causality \\
T0-3 & Pointwise (1$\times$1), no attention & Position-independent baseline \\
T0-4 & Dilated TCN WITH attention & Whether attention adds value \\
T0-5 & Conv WITH attention & Attention on non-causal conv \\
...
\begin{table}[h]
\centering
\caption{Tier 1 TCN Sweeps (3 seeds each)}
\begin{tabular}{lll}
\toprule
...
\begin{table}[h]
\centering
\caption{Experiment Budget Summary (TCN Pipeline)}
\begin{tabular}{lcc}
\toprule
...
\begin{enumerate}
    \item \textbf{Architecture comparison} (\texttt{fig\_architecture\_comparison.pdf}): Bar chart of $R^2$ (mean$\pm$std) across all TCN architecture variants (T0-1 through T0-6). Clearly shows dilated TCN without attention as best.

    \item \textbf{Error distribution} (\texttt{fig\_error\_distribution.pdf}): Violin/box plot of per-curve MAE across main architectures. Shows tail behavior and robustness.

    \item \textbf{J-V overlays (3$\times$3 grid)}: 9 representative curves, true vs predicted, showing best/median/worst reconstruction quality.

    \item \textbf{Ablation heatmap} (\texttt{fig\_ablation\_heatmap.pdf}): Rows = T0-1 through T0-10. Columns = $R^2$ median, MAE median, Voc error, Isc error. Normalized per column.

    \item \textbf{Attention impact} (\texttt{fig\_attention\_impact.pdf}): Paired comparison (with/without attention) for TCN and Conv architectures.
\end{enumerate}
...
\begin{enumerate}
    \item Batch size sweep: $R^2$ vs batch size (\texttt{fig\_batch\_size\_sweep.pdf})
    \item Epoch sweep: $R^2$ vs training epochs (\texttt{fig\_epoch\_sweep.pdf})
    \item Data scaling: 100k vs 100k+300k (\texttt{fig\_data\_scaling.pdf})
    \item Runtime comparison across architectures (\texttt{fig\_runtime\_comparison.pdf})
    \item Jacobian sensitivity heatmap: $|\partial\text{output}/\partial\text{input}|$ (features $\times$ voltage positions)
    \item Parameter perturbation tornado diagram (top-20 most sensitive parameters)
    \item Feature importance bar chart (Jacobian-based ranking)
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Dilated Causal TCN} (primary model): Significantly outperforms Split-Spline and standard NN approaches. Causal dilated convolutions on 8-point PCHIP voltage slices with Gaussian RBF positional encoding.
    \item \textbf{Key architectural finding}: Self-attention \textbf{hurts} performance on short (8-point) sequences; dilation provides sufficient receptive field.
...
The experiment plan (51 TCN runs + Split-Spline baselines) and figure list provide a clear roadmap for ICML submission. All experiments are compiled into a single SLURM script (\texttt{slurm\_tcn\_master\_pipeline.sh}).
