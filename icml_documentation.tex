...
\begin{enumerate}
    \item \textbf{Architecture comparison} (\texttt{fig\_architecture\_comparison.pdf}): Bar chart of $R^2$ (mean$\pm$std) across core architecture variants (T0-1 through T0-6). Highlights dilated Conv without attention as best.
    ...
    \item \textbf{Runtime comparison} (\texttt{fig\_runtime\_comparison.pdf}): Runtime and throughput across architecture variants.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Attention impact: paired comparison (with/without attention) for Conv and TCN (\texttt{fig\_attention\_impact.pdf})
